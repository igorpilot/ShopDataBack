...
class StoreService {
    ...
    async addProduct({storeId, deliveryId, productData, file}) {
        try {
            ...
            if (!productData.name || !Number.isFinite(productData.quantity) || !Number.isFinite(productData.purchasePrice)) {
                // throw new StoreServiceError('name, quantity, and purchasePrice are required and must be valid', 400);
                console.log('name, quantity, and purchasePrice are required and must be valid')
            }
    ...
    async changeProduct({productData, history, storeId, deliveryId, customerId, file}) {
        try {
            if (!storeId) {
            ...
            if (!productData.id || !productData.name || !Number.isFinite(productData.quantity) || !Number.isFinite(productData.purchasePrice)) {
            ...
            const productPayload = {
                ...
                quantity: Number(productData.quantity),
                purchasePrice: Number(productData.purchasePrice),
                sellingPrice: productData.sellingPrice ? Number(productData.sellingPrice) : undefined,
            ...
            // if (!productInRowsAll) {
            //     //throw new StoreServiceError('Product not found in rowsAll', 404);
            // }

            // Оновлення кількості в rowsAll залежно від deliveryId/customerId
            // if (productPayload.quantityDifference !== undefined) {
            //     if (deliveryId) {
            //         // Для rowsDelivery: quantityDifference прямо впливає на rowsAll
            //         productInRowsAll.quantity =  productInRowsAll.quantity + productPayload.quantityDifference
            //     } else if (customerId) {
            //         // Для rowsCustomer: quantityDifference має зворотний ефект (продаж зменшує, повернення збільшує)
            //         productInRowsAll.quantity = productInRowsAll.quantity - productPayload.quantityDifference
            //     }
            // }
            ...
            if (deliveryId) {
                ...
                const product = rowDelivery.products.find((p) => p.id === productPayload.id);
                if (!product) {
            ...
            if (customerId) {
                ...
                const product = rowCustomer.products.find((p) => p.id === productPayload.id);
                if (!product) {
