...
class UserService {
  ...
  async activate(activationLink) {
    ...
    if (!user) {
      throw ApiError.BadRequest("Неправильна силка активації");
    }
  ...
  async forgotPassword(email) {
    ...
    const resetLink = `${process.env.CLIENT_URL}/#/login?form=resetPassword&token=${resetToken}&email=${email}`;
    await MailService.sendMailForReset(email, resetLink);

    return {
      message: "Інструкції для відновлення пароля надіслані на вашу пошту",
    };
  }
